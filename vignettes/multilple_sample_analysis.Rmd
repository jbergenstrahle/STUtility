...
In general, most QC plots can be plotted with/without the spatial information added, i.e. the number of unique transcripts can be viewed as a histogram with capture-spot frequences or, the data points could be plotted as a heatmap with there spatial coordinates. In general, the extra layer of information subject to the spatial component can add valuable information (e.g. is the lower gene count a regional artifact?). As such, it's recommended to prefearbly use this as a basis for plotting - "it doesn't hurt".

### Scatter plots

For the sake of argument, lets start with a plot which is not suitable for adding the spatial component. An overall scatterplot can act as a simple measure of reproducibility between replicates. Here we display four replicates of consequative sections. 

```{r}
Idents(se) <- "sample" #For labels
avg.exp <- AverageExpression(se, return.seurat = F, verbose=T, add.ident = "sample", assay="SCT")

pairs(log(avg.exp$SCT), 
      panel=qc.scatter, lower.panel = NULL)

```

Sample 1,2 and 3,4 seems to resemble each other to a higher degree respectiviely. Wheater or not this is a technical artifact or a biological true difference is something one would look into. In this case, we are probably experiencing a batch effect between different sub-arrays.  
...
genelist <- c("geneA", "geneB", "geneC", "geneD", "geneE", "geneR", "geneT", "gene")
