...
se <- RunUMAP(se, reduction = "ica", dims = keep.dims)
...
se <- RunUMAP(object = se, dims = keep.dims, verbose = FALSE, n.components = 3, reduction = "ica", reduction.name = "umap.3d", reduction.key = "UMAP3D_")
...
ST.DimPlot(object = se, dims = 1:3, reduction = "umap.3d", blend = T, dark.theme = T, pt.size = 0.5, channels.use = c("blue", "red", "green"))
...
se <- FindNeighbors(object = se, dims = 1:20, verbose = FALSE, reduction = "NMF") %>%
  ...
  RunUMAP(reduction = "NMF", dims = 1:20, n.neighbours = 20, reduction.name = "umap.NMF", reduction.key = "UMAPNMF_")
