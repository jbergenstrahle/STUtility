...
OBS; The masking currently does _not_ work satisfactory for 10X visum slides
...
Remember, the labels include _all_ capture-spots, which by defaul have label "Default". To conduct DEA for the identity classes, we utilize the `FindMarkers()` function, and specify the two different levels which we want to compare against.
...
Exluding `ident.2` results in differential expression between one group vs all other capture-spots.
