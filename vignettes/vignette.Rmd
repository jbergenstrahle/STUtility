...
ST.DimPlot(se, dims = 1:8, ncol = 8, grid.ncol = 1, reduction = "ica", dark.theme = F, pt.size = 1, center.zero = T, palette = "RdYlBu")
```
...
The `DimOverlay()` can be used to overlay the RGB projection onto the H&E image. By default, a single dim.component is used and plotted as a heatmap:

```{r, fig.height = 8, fig.width = 8}

DimOverlay(se, reduction = "umap.3d", sample.index = 1, dims=c(1,2,3))

```

Setting the setting `blend=T` will combine the dimensional vectors

```{r, fig.height = 8, fig.width = 8}

DimOverlay(se, reduction = "umap.3d", sample.index = 1, dims=c(1,2,3), blend=T, pt.size=4)

```

The user can also choose to invert the colors, which can be performed on individual dimensional components, by adding a minus "-" in front of the vector. See more alternatives in the subsection [Playing around with colors]

Again, the `ProjectDim()` function can be used to display the genes that are most strongly correlated with the coordinate system. Note in the function call above that we defined `reduction.name`, which are subsequently stored in the Seurat object in the reduction slot: 
