...
Different dimensionality reductions methods are suported via Seurat (`RunPCA()`, `RunTSNE`, `RunICA()` ) and the output are stored in the Seurat object. 
...
[INSERT WAY TO FIND GENES A LA LUDLAR]
...
And the clustering using a modularity optimizer

```{r}
se <- FindClusters(object = se, verbose = FALSE)
```

We can then plot the clusters in two dimensions (e.g. via tSNE) spatially
...
```{r, fig.width = 6, fig.height = 6}
ST.DimPlot(object = se, label = TRUE, reduction = "tsne") + NoLegend() + ggtitle('sctransform') 
```

We can take a look at the top variable features and plot those on top of the tissues.

[OBS lagga in sa man forstar att se@assays$SCT@var.features alltid far senaste dim redcution?]
