...
As with all biological data, using multiple samples add power to the analysis and is a necessity to enable comprehensive insight which otherwise suffers from stochastic uncertainty. In this example tutorial, we display how you can input multiple samples, look for aggravating circumstances like batch effects and missing data, apply methods to correct such if present, get a holistic picture of your data as well as conduct more in depth analysis in various ways (more on this later). 
...
#REMOVE THIS FROM VIGNETTE
library(STutility)
library(Seurat)

#load("F:\JoeDocs\PhD\STUtility\testDatatoLoad.RData")
...
All the steps explained in this guide could be performed with only the expression data. However, the image data, apart from being fundamental to the biological interpretation, is used to filter out capture-spots that lies directly under the tissue. This filtering exludes the unwanted data points, lowering the memory burden of the data objects created as well as removing informational noise. 

An introductory animation is avaiable on our website: http://www.spatialtranscriptomicsresearch.org/

### Spot adjustment and selection

The gene expression data consists of a count matrix with genes in rows and capture "spots" in columns. Each spot represents a small area on an ST array from which the captured transcripts have been barcoded with a unique sequence. The Cy3 image gives an idea of how this grid system of spots is arranged in an ST array.

Bild på Cy3 - vi lar val ha nagon som bara har en snygg workflow bild att langa fram har?
...
Accordingly, after an ST experiment, we have the following three output files:
...
any number of _extra_ columns can be added with metadata. This iformation can then be used to e.g. coloring of plots and subsetting. These columns can be named as you like. 
...
```{r, eval=FALSE}
#REMOVE THIS LATER
infoTable <- read.table("metaData_mmBrain.csv", sep=";", header=T, stringsAsFactors = F)
```
...
Here, we demonstrate the creation of the seurat object, while also including some filtering by only keeping the genes that are found in at least 5 capture spots and a total count value >= 100. We also only keep the spots that contains >= 500 total transcrips. (Note: by default `prep.from.table()` will remove spots that are located outside of the tissue according to the infoTable. If your infoTable do not include paths to spotfiles, or, if you want to simply omit this step: set `spot.file=FALSE`). 
...
The core of an ST experiment is the interplay between quantative RNAseq data and qualative image data. Working in R, the handling of image information and plotting of multiple samples can get a bit cumbersome, especially if the images are of higher resolution. Typically, we rarely find much use of plotting multiple samples togheter with the tissue images within this framework. Instead, the general tip is to perform the various plots using only the expression data, and after finding anything of interest that warrants further analysis - take out that sample and plot it individually togheter with the stained tissue image. 
...
OBS: We have noted that some systems experience memory problems due to large size image information not being cleared when removing old obejcts. If this is the case, the user can set `time.resolve=TRUE`, which will add a small garbage collection step to avoid this issue.

NOTE: time.resolve is set to TRUE as deafult in the current version. This will imply that the process takes approx. 5sec longer per sample. Turn this off if annoyed. 

```{r, eval=FALSE}

se <- LoadImages(object = se, verbose = T, time.resolve = T)
#undrar om man inte ska ha den som default egentligen?
...
If you follow along the test set in this vignette, you will notice that the samples are not quite aligned with respect to each other on the array. We have implemented a process to align the images.
...
#OBS: vcgCreateKDtree not loaded by deafult (Rvcg package)
#computeTransform not loaded bby deafult (Morpho package)
#fixed by keywords internal ?
#Morpho måste läggas till som dependency, eller kanske suggested
...
The `type` parameter can be set to "masked", "raw" or "processed", to override the deafult use of processed masked images. 

```{r, fig.height = 6, fig.width = 12}

FeatureOverlay(object = se, features = c("Nrgn", "Cck"), 
               palette = "GrRd", sample.index = 5,
               type="raw")
...
Following the rationale expressed above, we transform the data according to the Seurat workflow. Note: for comprehensive tutorials in the different options and workflow possitibilites avaiable within Seurat, we recommend looking at their website https://satijalab.org/seurat/  
...
## Finding spatial expression pattens and areas of interest

The strength of untargeted whole transcriptome capture is the ability to perform unsupervised analysis and the ability to find spatial gene expression patterns. We've found good use of the dimensionally reduction method "ICA" (Independent Component Analysis) to find spatial expression pattens. An example is demonstrated below:
...
Different dimensionality reductions methods are suported via Seurat (`RunPCA()`, `RunTSNE`, `RunICA()`, `runUMAP()` ) and the output are stored in the Seurat object. 
...
To extract the genes that drives the separtion accoring to the dimensionallity reduction, we can use the following function:
...
Where we specify the dimensions of interest (here IC_1 and IC_3, which are regions that seems to confer a clear spatial expression histology. 

### Clustering

Clustering is a standard procedure in genomic analysis, and the methods for doing so are numerous. Here we demonstrate an example where we use the result of ICA to perform clustering. In the priveous section, we demonstrated how to plot a subset of dimensions from the ICA output. Going through this list, we can notice the dimensions that are "spatially active", i.e. that seems to confer a spatial pattern along their axis. We can extract these dimensions:

```{r, fig.width=16, fig.height=10}
keep.dims <- c(1:12, 16:18, 21:28, 30:34, 40:41, 47, 49:50)
```

And then use them to construct a Shared Nearset Neighbor (SSN) Graph. 

```{r, eval=FALSE}
se <- FindNeighbors(object = se, dims = keep.dims, verbose = FALSE, reduction = "ica")
```

And then clustering using a modularity optimizer

```{r, eval=FALSE}
se <- FindClusters(object = se, verbose = FALSE)
```

We can plot the clusters spatially
...
One approach to visualize the result of dimensionallity reduction is to use the first three dimensions and transfrom the values into RGB color space. This 3dim space can then be utilized for spatial visualization.
We demonstrate the teqnique with UMAP, using our ICA dimensions as input:
...
Once, again the `ProjectDim()` function is used to display the genes that are most strongly correlated with the coordinate system. Note in the function call above that we used our own defined `reduction.name`, which are subsequently stored in the Seurat object. 
...
As seen in the plot above, there are clear spatial patterns that are confirm by the histology of the tissue. 
Lets take an example, looking at the first UMAP coordinate (red color) - we note a clear patten [bra beskrivning av vad vi ser]

Lets use the Allen brain atlas to see how well our data match. Lets take the gene with highest correlation to the red color, i.e. "Cck". 

https://mouse.brain-map.org/gene/show/12209

Expression: [Expression data - Allen brain atlas](Allen_expression_data.PNG)

ISH: [ISH data - Allen brain atlas](Allen_ISH_data.PNG)
...
We start off by doing a simple scatter correlation plot of the average expression across the samples. In this case, we are looking at 8 consequtive tissue sections. Accordingly, they should display a quite high correlation across all samples. 
...
The dimensionallity reduction (e.g. via UMAP) projection onto RGB space works excellent as a batch effect check. Here we show four samples of mouse hippocampus that clearly display a batch effect subject to samples prepared on different arrays:
...
Once again, we make use of the Seurat 

https://satijalab.org/seurat/v3.0/de_vignette.html

Lets use the RGB UMAP strategy to select a region of interest. 

```{r}

#Eventuellt interaktiv Rplot
gg <- ST.DimPlot(object = se, dims = 1:3, 
           reduction = "umap.3d", blend = T, 
           dark.theme = T,
           delim="x|_")

test <- ggplot_build(gg) 
rgbTransform <- as.data.frame(t(col2rgb(test$data[[1]]$colour)))
gg$layers[[1]]$mapping$rgbs <- paste("R:", round(rgbTransform$red/2.55), 
                                          "% G:", round(rgbTransform$green/2.55), 
                                          "% B:", round(rgbTransform$blue/2.55), "%",
...
se$rgb.red <- round(rgbTransform$red/2.55)
se$rgb.green <- round(rgbTransform$green/2.55)
se$rgb.blue <- round(rgbTransform$blue/2.55)

ggP <- ggplotly(gg , tooltip="rgbs")

```

Example: look at the inner section of (the hundora). Our hypothesis is that there is a difference between the uppern and lower part, displed by the dark orange and the more yellow part. It might even be so that across time, one expression program would turn to the next (obs, we dont have a time serier here). Lets conduct a differential expression between these two regions, or rather "colors". We can define a color interval to perform our selection, guided by the hover information we obtain from the picture. Lets define the orange cells as:  

Red: >85%, Green: 30-40%, Blue: 10-20%.

And the yellow cells as:

Red: >85%, Green: 65-75%, Blue: 20-30%. 

```{r}

#Put func call inside e.g. STDimplot

rgb.label <- function(object, 
                       label1 = "label1",
                       label2 = NULL,
                       red1=c(0,255), 
                       green1=c(0,255), 
                       ...
                       blue2=c(0,255)){
  object$rgbInfo <- 0
  object$rgbInfo[which(se$rgb.red > red1[1] & se$rgb.red < red1[2]
& se$rgb.green > green1[1] & se$rgb.green < green1[2]
& se$rgb.blue > blue1[1] & se$rgb.blue < blue1[2])] <- label1
    
 if(!is.null(label2)){
    object$rgbInfo[which(se$rgb.red > red2[1] & se$rgb.red < red2[2]
  & se$rgb.green > green2[1] & se$rgb.green < green2[2]
  & se$rgb.blue > blue2[1] & se$rgb.blue < blue2[2])] <- label2
 }
...
se <- rgb.label(object = se, label1="orange", red1=c(85,255), green1=c(30,40), blue1=c(10,20),
                label2 = "yellow", red2=c(85,255), green2=c(67,75), blue2=c(20,30))
...
```{r}
de.markers <- FindMarkers(se, ident.1 = "orange", ident.2 = "yellow")

head(de.markers)
```

We get the values for group1 vs group2, i.e. "Wfs1" is determined to be overexpressed in the yellow area vs the orange, i.e (lower region of the [insert anatomical area].  

`pct.1` and `pct.2` is the percentage of capture-spots where the gene is detected in the first and second group. 

Lets check the spatial distribution of Wfs1:

```{r}

MultiFeatureOverlay(se, features = "Wfs1", 
                    sampleids = paste0(1:8),
                    palette = "GrRd",
                    method="raster")
```

In 6 out of 8 samples, we have a clear "stroke" of expression in the lower part of the [insert anatomical area]

# AAAAADDD det vore snyggt om man kunde VISUALISERA de REGIONER MAN VALT SO MAN VET !

You can exlude `ident.2` to instead perform differential expression between one group vs all other capture-spots.

```{r}
de.markers <- FindMarkers(se, ident.1 = "orange")
head(de.markers)
#OK DETTA E INTE SA JAKLA SNYGGT IOF ATT MAN ENDAST KAN GE 2 ST AT GANGNE, ERE OK ELLER INTE?
```

Lets check the top gene based on adjusted p-value:

```{r}

MultiFeatureOverlay(se, features = "Pou3f1", 
                    sampleids = paste0(1:8),
                    palette = "GrRd",
                    method="raster")
```
...
The core of STUtility is the Seurat object, where all information from the different functions are saved. Accordingly, it's convinient to extract this information by opeing the different slots within the object. See https://satijalab.org/seurat/ for extensive information.

### What if I don't have spot file coordinates?

No problem. However, the count matrix needs to have some sort of coordinate info in order to actually plot the data spatially. Normally this information is kept in the colnames of the count matrix, e.g. "XxY" with X=xcoordiante and Y=ycoordinate. Creating the Seurat object with the `prep.from.table` function will result in the addition of "_z" to the columnames of the input matrix, where "z" is the sample number. When calling the spatial plots (i.e. ST.xxxPlot), use the parameter `delim` to specifiy the type of colnames you have in the Seurat object. If the names are according to the outline above, this would imply `delim = "x|_"`.  

### Saving of images 

For saving images, we usally use the built in function `pdf()` within R. All ploting functions in STutility returns a ggplot object. Accordingly just, initiate the pdf function, make the plot and then turn off the graphic device:
...
STUtility relies heavily on the Seurat package, currently version 3.0 As such, we would like to take the opportunity to thank the excellent folks at Satija Lab for their efforts and contribution to the space. The bioinformatic folks at Scilifelab working on Spatial Transcriptomics from the early days of it's inception have gone over numerous ways of handling this type of data in our neverending pursuit of placing transcriptomic information in its spatial context. So far, the Seurat approach to transformation demonstrated here is our preferred method. Furhermore, we picture that the suitability will not deterioate when the capture spot resolution of the spatial methods keeps getting smaller and smaller in size, eventually reaching the single-cell level that Seurat in reality is developed for to begin with. 
