...
We are now ready to load our samples and create a "seurat" object. Note: type `?CreateSeuratObject()` to view the parameters that can be used to directly filter the input data. In this regard, the seruat definition of "cells" are here eqvivalent to "capture spots". 

Here, we demonstrate the creation of the seurat object, while also including some filtering by only keeping the genes that are found in at least 5 capture spots.  
